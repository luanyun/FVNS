clear
global strings1 intervalTime citys
load data.mat
...
j1=j1(1:Nflight);
for i=2:Nflight
    if schedules1(i,4)~=schedules1(i-1,4)%first flight won't delay
        continue;
    end
...
while t~=inf
    ...
    for j=1:numac
        ...
        swapj=find(strings1{j}(:,9)<t2-intervalTime&strings1{j}(:,3)==strings1{ac}(i,2),1,'last');
        if isempty(swapj)
            continue;
        end
        flight=strings1{ac}(i,1);
        kflight=flight==schedules(:,2);
        jTime=max(strings1{ac}(i-1,9)+intervalTime,strings1{j}(swapj+1,5));
        acTime=max(strings1{j}(swapj,9)+intervalTime,strings1{ac}(i,5));
        ...
        if acTime<strings1{ac}(i,8)
            ...
            newcost=delaycost(strings1{ac},i,acTime,intervalTime)+delaycost(strings1{j},swapj+1,jTime,intervalTime);
    ...
    if best>0
        substring=strings1{ac}(i:end,:);
        strings1{ac}(i:end,:)=[];strings1{ac}(1:i-1,8);
        strings1{ac} =[strings1{ac};strings1{jbest}(sjbest:end,:)];
        strings1{jbest}(sjbest:end,:) = [];
        strings1{jbest}=[strings1{jbest};substring];
        tkStrings(ac)=jTimeBest;
        tkStrings(jbest)=acTimeBest;
        reschedule(ac,i,jTimeBest);
        reschedule(jbest,sjbest,acTimeBest);
        continue
    end
